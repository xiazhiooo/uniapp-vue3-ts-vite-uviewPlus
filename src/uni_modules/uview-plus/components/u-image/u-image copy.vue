<template>
  <u-transition mode="fade" :show="show" :duration="fade ? 1000 : 0">
    <view
      ...
      class="u-image"
      @tap="onClick"
      :style="[wrapStyle, backgroundStyle, { height: height == '225' ? 'none' : $u.addUnit(height) }]"
    >
      <image
        :src="src"
        :mode="mode"
        ...
        :style="{
          ...
          height: showLoading && loading ? 'none' : $u.addUnit(height),
        }"
      ></image>
    </view>
    <view
...
export default {
  ...
  computed: {
    wrapStyle() {
      ...
      // 错误文件显示默认高度
      if (this.showError && this.isError && !this.loading) {
        style.height = this.$u.addUnit(this.height);
      } else if (this.showLoading && this.loading) {
        style.height = this.$u.addUnit(this.height);
      } else {
        style.height = "none";
      }
